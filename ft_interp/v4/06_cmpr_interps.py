...
def main():
    ...
    data_files = [
        Path(r'orig/precipitation_kriging_1km_orig.nc'),
        Path(r'ifted/precipitation_kriging_1km_ifted.nc'), ]

    data_dss = ['OK', 'OK']
    data_labs = ['orig', 'ifted']

    stn_data_file = Path(r'../precipitation.csv')
    stn_crds_file = Path(r'../precipitation_coords.csv')
    ...
    stn_data_df.index = pd.to_datetime(stn_data_df.index, format=time_fmt_csv)
