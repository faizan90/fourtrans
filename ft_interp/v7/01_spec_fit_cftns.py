...
#     ift_vals = np.fft.irfft(fft_vals, axis=0)
    ...
    cos_spec_df.to_csv(str(cos_out_dir / f'cos.csv'), sep=sep)
    sin_spec_df.to_csv(str(sin_out_dir / f'sin.csv'), sep=sep)
...
#     plt.show()
    ...
    with open(str(cos_out_dir / f'vg_strs.csv'), 'w') as txt_hdl:
        txt_hdl.write(f'freq;vg\n')

        for i in range(n_freqs):
            txt_hdl.write(f'{i};{phs_cftn_str}\n')

    with open(str(sin_out_dir / f'vg_strs.csv'), 'w') as txt_hdl:
        txt_hdl.write(f'freq;vg\n')
