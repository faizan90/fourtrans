...
def main():
    ...
    data_files = [
        Path(r'mag/precipitation_kriging_1km_mag.nc'),
        Path(r'phs/precipitation_kriging_1km_phs.nc'), ]

    data_dss = ['OK', 'OK']
    data_labs = ['mag', 'phs']

    stn_data_files = [Path(r'mag.csv'), Path(r'phs.csv')]
    stn_crds_file = Path(r'../precipitation_coords.csv')

    vg_str_files = [
        Path(r'mag/vg_strs.csv'), Path(r'phs/vg_strs.csv')]
