...
def cmpt_cftn(data_file, crds_file, out_dir, ref_mag_vg_str, ref_phs_vg_str):
...
#
#     plt.savefig(
#         str(out_dir / f'phs_corr_pcm.png'), bbox_inches='tight')
#
# #     plt.show()
#     plt.close()
#
...
#
#     plt.savefig(
#         str(out_dir / f'mag_corr_pcm.png'), bbox_inches='tight')
#
# #     plt.show()
#     plt.close()
...
#     plt.ylim(min_corr, max_corr)
    ...
    plt.savefig(
        str(out_dir / f'phs_corr_sctr.png'), bbox_inches='tight')
...
#     plt.ylim(min_corr, max_corr)
    ...
    plt.savefig(
        str(out_dir / f'mag_corr_sctr.png'), bbox_inches='tight')
...
def main():
    ...
    main_dir = Path(
#         r'P:\Synchronize\IWS\Testings\fourtrans_practice\multisite_phs_spec_corr\mag_cos_sin_interp\precipitation_kriging')
...
#         r'P:\Synchronize\IWS\Testings\fourtrans_practice\multisite_phs_spec_corr\mag_and_phss_interp\precipitation_kriging__better_vgs')
        ...
        r'P:\Synchronize\IWS\Testings\fourtrans_practice\multisite_phs_spec_corr\mag_and_phss_interp\precipitation_kriging__better_vgs__10_nebs')

    os.chdir(main_dir)

    in_data_files = [Path(r'orig/ts_OK.csv'), Path(r'ifted/ts_OK.csv')]
    in_crds_files = [Path(r'orig/ts_crds.csv'), Path(r'ifted/ts_crds.csv')]

    cftns_file = Path(r'../../precipitation_cftns.csv')

    out_dirs = [data_file.parents[0] for data_file in in_data_files]

    cftns_df = pd.read_csv(cftns_file, sep=';', index_col=0, squeeze=True)

    ref_mag_vg_str = cftns_df.loc['mag']
    ref_phs_vg_str = cftns_df.loc['phs']

    for data_file, crds_file, out_dir in zip(
        in_data_files, in_crds_files, out_dirs):

        print('Going through:', data_file, crds_file, out_dir,)
