...
from cyth import (OrdinaryKrigingFT)  # , SimpleKriging, ExternalDriftKriging_MD)
...
def ordinary_kriging(ok_vars):
    ...
    curr_x_coords = in_stns_coords_df.loc[:, 'X'].values
    curr_y_coords = in_stns_coords_df.loc[:, 'Y'].values

#     ord_krig = OrdinaryKrigingFT(
#         xi=curr_x_coords,
#         yi=curr_y_coords,
#         xk=curr_x_coords[[0]] + 100,
#         yk=curr_y_coords[[0]] + 100,
#         model=model)
    ...
    wts = ord_krig.lambdas

    nan_fld = np.ones(fin_cntn_idxs.shape[0], dtype=bool)
#     nan_fld = np.zeros(fin_cntn_idxs.shape[0], dtype=bool)
#     nan_fld[fin_cntn_idxs] = 1

    nnan_idxs = np.where(nan_fld.reshape(krige_coords_orig_shape))
    for i in range(krige_x_coords_mesh.shape[0]):
        ord_krige_flds[:, nnan_idxs[0][i], nnan_idxs[1][i]] = (
            in_data_df.values * wts[i, :]).sum(axis=1)
