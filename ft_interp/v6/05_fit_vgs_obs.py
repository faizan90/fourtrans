...
def get_ppt_paths():
    ...
    in_vals_df_loc = Path(os.path.join(
             r'neckar_1min_ppt_data_20km_buff_Y2009__RR5min_RTsum.pkl'))

    in_stn_coords_df_loc = Path(os.path.join(
             r'metadata_ppt_gkz3_crds_subset.csv'))

    out_dir = Path(r'ppt_kg')
...
def main():
    ...
    strt_date = '2009-01-01 00:00:00'
    end_date = '2009-12-31 23:59:00'
    min_valid_stns = 10
...
#     drop_stns = ['P3733', 'P3315', 'P3713', 'P3454']
    ...
    for vg_var in vg_vars:
        ...
        elif in_vals_df_loc.suffix == '.pkl':
            in_vals_df = pd.read_pickle(in_vals_df_loc)

        else:
            raise NotImplementedError(
                f'Unknown extension of in_data_file: {in_vals_df_loc.suffix}!')
        ...
        in_coords_df = in_coords_df[['X', 'Y']].astype(float)
