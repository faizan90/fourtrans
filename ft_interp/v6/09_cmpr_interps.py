...
def main():
    ...
    data_files = [
        Path(r'neckar_ppt_2009_5min.h5'),
        Path(r'neckar_ppt_2009_5min.h5'), ]

    data_dss = ['kg_1km/OK/1', 'kg_1km/OK/1']
    data_labs = ['ifted', 'ifted']

    stn_data_file = Path(r'../neckar_1min_ppt_data_20km_buff_Y2009__RR5min_RTsum.pkl')
    ...
    x_crds_lab = 'x_cen_crds/1'
    y_crds_lab = 'y_cen_crds/1'

    out_dir = Path(r'cmpr_figs__interp')
    ...
    for data_file, data_ds in zip(data_files, data_dss):
        with h5py.File(data_file, 'r') as h5_hdl:

            if datas is None:
                datas = []

                datas.append(h5_hdl[data_ds][...])

            else:
                assert np.all(h5_hdl[data_ds].shape == datas[-1].shape)

                datas.append(h5_hdl[data_ds][...])

            if time_step_ds is None:
                time_step_ds = h5_hdl[time_var]

                time_steps = pd.to_datetime(
                    time_step_ds[:], format='%Y%m%dT%H%M%S')

            if x_crds is None:
                x_crds = h5_hdl[x_crds_lab][...]
                y_crds = h5_hdl[y_crds_lab][...]

            else:
                assert not np.setdiff1d(
                    x_crds, h5_hdl[x_crds_lab][...]).size

                assert not np.setdiff1d(
                    y_crds, h5_hdl[y_crds_lab][...]).size
...
#         (y_crds - (y_cell_size * 0.5), [y_crds[-1] + y_cell_size]))
    ...
    x_crds_mesh, y_crds_mesh = x_crds, y_crds
