...
def main():
    ...
    ft_sub_all = np.zeros_like(ft)

    for mag_idx in mag_idxs[:2]:
        ft_sub = np.zeros_like(ft)

        rand_phs = -np.pi + ((2 * np.pi) * np.random.random())

        ft_sub.real[mag_idx] = mag_spec[mag_idx] * np.cos(rand_phs)
        ft_sub.imag[mag_idx] = mag_spec[mag_idx] * np.sin(rand_phs)

        ft_sub_all[mag_idx] = ft_sub[mag_idx]
...
#     plt.show()
    ...
    plt.savefig('test_four_trans_example_phs_rand.png', bbox_inches='tight')
    plt.close()
