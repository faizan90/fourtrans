...
def main():
    ...
    # weight of reference data ft.
    ratio_ab = 0.3
    pow_ab = 1.0

    out_dir = Path(r'iaaft_test_ms_mixed2_31')
    #==========================================================================
    ...
    ref_ft[0,:] = 0
    # ref_ft /= ((np.abs(ref_ft) ** 2).sum(axis=0) ** 0.5)
    ...
    ref_ft_ranks[0,:] = 0
    # ref_ft_ranks /= ((np.abs(ref_ft_ranks) ** 2).sum(axis=0) ** 0.5)
        ...
        for m in range(n_repeat):
            ...
            if True:
                ...
                for i in range(max_opt_iters):
                    ...
                    # ##
                    sim_ift = (((ratio_ab ** pow_ab) * sim_ift_a) + (((1 - ratio_ab) ** pow_ab) * sim_ift_b))
