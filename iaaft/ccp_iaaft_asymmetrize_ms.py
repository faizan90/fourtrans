...
def main():
    ...
    out_dir = Path(r'test_asymmetrize_ms_19')
    ...
    n_levels_bds = np.array([1, 200])
    max_shift_exps_bds = np.array([0.5, 10])
    max_shifts_bds = np.array([1, 20])
    pre_vals_ratios_bds = np.array([0.95, 0.999])
    asymm_n_iters_bds = np.array([1, 2])
    prob_centers_bds = np.array([0.0, 0.05])
    ...
    marg_ratios = (
            ...
            ((marg_ratio_bds[1] - marg_ratio_bds[0]) *
             np.random.random(size=(n_sims, n_cols))))

    marg_ratios[:, 1] = 1 - marg_ratios[:, 0]

    n_levels = np.random.randint(
        n_levels_bds[0], n_levels_bds[1] + 1, size=(n_sims, n_cols)
        ).astype(np.uint32)

    max_shift_exps = (
        max_shift_exps_bds[0] +
        ((max_shift_exps_bds[1] - max_shift_exps_bds[0]) *
         np.random.random(size=(n_sims, n_cols))))

    max_shifts = (
        max_shifts_bds[0] +
        ((max_shifts_bds[1] - max_shifts_bds[0]) *
         np.random.random(size=(n_sims, n_cols)))).astype(np.int64)

    pre_vals_ratios = (
        pre_vals_ratios_bds[0] +
        ((pre_vals_ratios_bds[1] - pre_vals_ratios_bds[0]) *
         np.random.random(size=(n_sims, n_cols))))

    asymm_n_iters = np.random.randint(
        asymm_n_iters_bds[0],
        asymm_n_iters_bds[1] + 1,
        size=(n_sims, n_cols)).astype(np.uint32)

    prob_centers = (
        prob_centers_bds[0] +
        ((prob_centers_bds[1] - prob_centers_bds[0]) *
         np.random.random(size=(n_sims, n_cols))))
    #==========================================================================
    ...
    args_gen = (
        (ref_data_cls,
         ...
         sim_lab,
         (n_levels[sim_idx,:],
          max_shift_exps[sim_idx,:],
          max_shifts[sim_idx,:],
          pre_vals_ratios[sim_idx,:],
          asymm_n_iters[sim_idx,:],
          prob_centers[sim_idx,:]),
         asymmetrize_flag,
         asymms_rand_err,
        )
    ...
    sim_labs = []
    for sims, order_sdiffs, asymm_props in ress:
        for col in cols:
...
def get_sim_dict(args):
    ...
    return sims, order_sdiffs, asymm_props


def asymmetrize_data(
        sim_data,
        sim_probs,
        asymm23_props,
        ref_data_cls,
    ...
    (n_levelss,
     ...
     pre_vals_ratios,
     asymm_n_iterss,
     prob_centers) = asymm23_props

    asymm_data = asymmetrize_type_5_ms_cy(
        # sim_data,
        sim_probs,
        sim_probs,
        n_levelss,
    ...
    sim_data_new = np.empty_like(sim_data)
    for i in range(asymm_data.shape[1]):
        sim_data_new[:, i] = ref_data_cls.data_sort[
            np.argsort(np.argsort(asymm_data[:, i])), i]
