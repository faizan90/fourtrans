...
def main():
    ...
    out_dir = Path(r'test_asymm23_dis_21___opt')
    ...
    np.random.seed(3432423432)  # To check consistency of the best solution.
    ...
    asymms_rand_err = (-1e-6) + (2e-6) * np.random.random(size=data_sort.shape[0])
    #==========================================================================
    ...
    opt_res = differential_evolution(
        ...
        maxiter=1000,
        updating='deferred',
        integrality=np.array([True, False, True, False], dtype=bool))
...
def obj_ftn(asymm23_props, args):
    ...
    data_rand = data_rand.copy()
    order_old = order_old.copy()
    order_old_ranks = order_old_ranks.copy()
    #==========================================================================
    ...
    obj_val_global_min = get_obj_val(
        ref_data_cls, data_rand, ratio_a, ratio_b, test_opt_flag,)
    ...
    for i_repeat in range(n_repeat):
        ...
        if asymmetrize_flag:
            asymmetrize_data(data_rand, asymm23_props, asymms_rand_err)
        #======================================================================
        ...
        if ratio_a and auto_spec_flag:
            ...
            for k in range(len(cols)):
                sim_ift_a_auto[:, k] = data_sort[order_new_a[:, k], k].copy()
        ...
        if ratio_a and cross_spec_flag:
            ...
            for k in range(len(cols)):
                sim_ift_a_cross[:, k] = data_sort[order_new_a[:, k], k].copy()
        ...
        data_rand = np.empty_like(data)
        for k in range(len(cols)):
            data_rand[:, k] = data_sort[order_old[:, k], k].copy()
        #======================================================================
    ...
    if obj_val_global_min < 0.2:
        print(asymm23_props, obj_val_global_min, obj_val, i_data_best)

    return obj_val_global_min
...
def get_sim_dict(args):
    ...
    mag_ratios_adj_exp = 0.5

    ref_mag_sum = (ref_mag[1:] ** mag_ratios_adj_exp).sum()
    ref_mag_ranks_sum = (ref_mag_ranks[1:] ** mag_ratios_adj_exp).sum()
    ...
    for i_repeat in range(n_repeat):
        ...
        if ratio_a:
            ...
            if i_repeat and adjust_mag_diffs_flag:
                ...
                # This doesn't work when asymmetrize flag is on.
                ...
                mag_ratios = (
                    (ref_mag ** mag_ratios_adj_exp) /
                    (sim_mag ** mag_ratios_adj_exp))[1:]

                ref_mag_ratio_sum = (
                    (ref_mag[1:] ** mag_ratios_adj_exp) * mag_ratios).sum()

                mag_ratios *= ref_mag_sum / ref_mag_ratio_sum

                mag_ratios = np.concatenate(([1.0], mag_ratios.ravel()))

                ref_mag_adj = ref_mag * mag_ratios.reshape(-1, 1)
        ...
        if ratio_b:
            ...
            if i_repeat and adjust_mag_diffs_flag:
                ...
                mag_ratios = (
                    (ref_mag_ranks ** mag_ratios_adj_exp) /
                    (sim_mag_ranks ** mag_ratios_adj_exp))[1:]

                ref_mag_ranks_ratio_sum = (
                    (ref_mag_ranks[1:] ** mag_ratios_adj_exp) *
                    mag_ratios).sum()

                mag_ratios *= ref_mag_ranks_sum / ref_mag_ranks_ratio_sum

                mag_ratios = np.concatenate(([1.0], mag_ratios.ravel()))

                ref_mag_ranks_adj = ref_mag_ranks * mag_ratios.reshape(-1, 1)
...
def asymmetrize_data(sim_data, asymm23_props, asymms_rand_err=None):
...
def get_obj_val(ref_data_cls, sim_data, ratio_a, ratio_b, test_opt_flag=False):
    ...
    if (sim_data.shape[0] > 1) and ratio_b:
