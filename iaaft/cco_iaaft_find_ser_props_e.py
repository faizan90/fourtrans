...
def main():
    ...
    out_dir = Path(r'test_asymm23_dis_23__01')
    ...
    quant_levels = np.arange(5, 201, 5)
    shift_exps = np.linspace(1, 10, 21)
    max_shifts = np.arange(1, 4)
    pre_vals_ratios = np.linspace(0.2, 1.0, 10)
    asymmetrize_loop_range = np.arange(1, 4)

    # quant_levels = np.arange(60, 81, 5)
    # shift_exps = np.linspace(6, 8, 11)
    # max_shifts = np.arange(1, 3)
    # pre_vals_ratios = np.linspace(0.8, 1.0, 9)
    # asymmetrize_loop_range = np.arange(1, 3)
    ...
    asymms_rand_err = (
        (-1e-6) + (2e-6) * np.random.random(size=data_sort.shape[0]))
    #==========================================================================
...
def get_sim_dict(args):
    ...
    for i_repeat in range(n_repeat):
        ...
        if asymmetrize_flag and (i_repeat == (n_repeat - 2)):
        # if asymmetrize_flag and (obj_val > obj_val_global_min):
            for _ in range(asymm23_props[4]):
    ...
    print(
        ...
        f'with order_sdiff: {order_sdiff:0.3f} and '
        f'with obj_val_global_min: {obj_val_global_min:0.3f}')
...
def asymmetrize_data(sim_data, asymm23_props, asymms_rand_err=None):
    ...
    n_levels, max_shift_exp, max_shift, pre_vals_ratio, _ = asymm23_props
