...
def main():
    ...
    # Wether to keep scorrs or pcorrs.
    keep_scorrs_flag = True
    # keep_scorrs_flag = False

    out_dir = Path(r'iaaft_test_ms_04')
    #==========================================================================
    ...
    for j in range(n_sims):
        ...
        # sim_phs = ref_phs_diffs.copy()
        #
        # sim_phs += -np.pi + (
        #     (2 * np.pi) * np.random.random(size=(ref_phs.shape[0], 1)))
        #
        # sim_phs[0,:] = ref_phs[0,:]
        #
        # sim_ft_new = np.empty_like(ref_ft)
        #
        ...
        for k in range(len(cols)):
            # Correlated.
            # order_old[:, k] = np.argsort(np.argsort(sim_ift[:, k]))
