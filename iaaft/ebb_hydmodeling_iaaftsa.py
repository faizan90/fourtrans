...
def main():
    ...
    sim_file = Path('hydmod_tss/test_hbv_all_06__mult_prms_search.h5').absolute()

    main_hyd_mod_dir = Path(f'hydmod/{sim_file.stem}').absolute()

    h5_dss = [f'sim_data_{i:02d}' for i in range(0, 16)]
    ...
    mp_args = (
        (str(main_hyd_mod_dir / h5_ds), h5_ds, overwrite_flag, sim_file)
        for h5_ds in h5_dss)

    n_cpus = 3

    main_hyd_mod_dir.mkdir(exist_ok=True)
    ...
    if n_cpus == 1:
        ...
        list(mp_pool.map(hydmod_iaaft, mp_args, chunksize=1))

    return


def hydmod_iaaft(args):

    hyd_mod_dir, h5_ds, overwrite_flag, h5_file = args

    print(h5_ds)

    if (not overwrite_flag) and hyd_mod_dir.exists():
    ...
    in_ini_file = (
        Path(__file__).parents[0] / r'zab_iaaftsa_hydmodeling_config.ini')
    ...
    if optimize_flag:
        ...
        # always in cumecs
        # obs_q_file = h5_file  # cfp['OPT_HYD_MODEL']['obs_q_file']
        #
        # ppt_file = h5_file  # cfp['OPT_HYD_MODEL']['ppt_file']
        # ppt_ds_grp = h5_ds  # cfp['OPT_HYD_MODEL']['ppt_ds_grp']
        #
        # temp_file = h5_file  # cfp['OPT_HYD_MODEL']['temp_file']
        # temp_ds_grp = h5_ds  # cfp['OPT_HYD_MODEL']['temp_ds_grp']
        #
        # pet_file = h5_file  # cfp['OPT_HYD_MODEL']['pet_file']
        # pet_ds_grp = h5_ds  # cfp['OPT_HYD_MODEL']['pet_ds_grp']
        ...
        assert len(prcss_cats_ints_list) == 1

        all_vars_dict = get_h5_tss(
            h5_file,
            h5_ds,
            h5_beg_times,
            h5_end_times,
        ...
        in_ppt_dfs_dict[prcss_cats_ints_list[0]] = all_vars_dict['prec']
        in_temp_dfs_dict[prcss_cats_ints_list[0]] = all_vars_dict['temp']
        in_pet_dfs_dict[prcss_cats_ints_list[0]] = all_vars_dict['pet']

        in_q_df = all_vars_dict['q_obs']

        in_q_df.columns = prcss_cats_ints_list

        in_cell_vars_dict = get_cell_vars_iaaft(h5_file, prcss_cats_ints_list)
        ...
        if np.any(all_prms_flags[:, 1]):
            aux_cell_vars_dict['lulc_ratios'] = get_cell_vars_iaaft(
                lulc_file, lulc_ds_grp)[lulc_ds_grp]

        if np.any(all_prms_flags[:, 2]):
            aux_cell_vars_dict['soil_ratios'] = get_cell_vars_iaaft(
                soil_file, prcss_cats_ints_list, soil_ds_grp)[soil_ds_grp]

        if np.any(all_prms_flags[:, 3]) or np.any(all_prms_flags[:, 5]):
            aux_cell_vars_dict['aspect'] = get_cell_vars_iaaft(
                aspect_file, prcss_cats_ints_list, aspect_ds_grp)[aspect_ds_grp]

        if np.any(all_prms_flags[:, 4]) or np.any(all_prms_flags[:, 5]):
            aux_cell_vars_dict['slope'] = get_cell_vars_iaaft(
                slope_file, prcss_cats_ints_list, slope_ds_grp)[slope_ds_grp]
...
def get_h5_tss(
    ...
    with h5py.File(path_to_h5, mode='r', driver=None) as h5_hdl:
        ...
        keys = [key for key in data_ds.keys()]
        ...
        for key in keys:
            ...
            if set_na_to_zero_flag and (key == 'prec'):
...
def get_cell_vars_iaaft(path_to_h5, prcss_cats_ints_list, extra_ds=None):
    ...
    with h5py.File(path_to_h5, mode='r', driver=None) as h5_hdl:
        keys = prcss_cats_ints_list

        for key in keys:
            rows = np.array([0.0])
            cols = np.array([0.0])
            key_area_ratios = np.array([1.0])
