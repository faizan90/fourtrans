...
def get_ranks(in_arr):

    return rankdata(in_arr)
...
#     phs_spec_shift += phs_shift_rads
    ...
    data_shift_arr = np.fft.irfft(ft_shift)

    fig, axs = plt.subplots(nrows=2, ncols=3, figsize=(20, 10))

    axs[0, 0].plot(in_arr, alpha=0.9, label='input')
    axs[0, 0].plot(data_shift_arr, alpha=0.9, label='output')
    axs[0, 0].grid()
    axs[0, 0].legend()
...
#     axs[1, 1].legend()
    ...
    in_arr_ranks = get_ranks(in_arr)
    data_shift_ranks = get_ranks(np.round(data_shift_arr, 6))

    axs[0, 1].scatter(in_arr_ranks, np.roll(in_arr_ranks, 10), alpha=0.7)
    axs[1, 1].scatter(
        data_shift_ranks, np.roll(data_shift_ranks, 10), alpha=0.7)
